...
SEXP merge_db(SEXP sxpdb1, SEXP sxpdb2) {
  ...
  try{
    ...
    Rf_error("Error merging database %s into %s: %s\n", db2->configuration_path().string().c_str(), db1->configuration_path().string().c_str(), e.what());
  }
...
SEXP path_db(SEXP sxpdb) {
  ...
  SEXP res = PROTECT(Rf_mkString(path.string().c_str()));
...
SEXP values_from_calls(SEXP sxpdb, SEXP pkg, SEXP fun) {
  void* ptr = R_ExternalPtrAddr(sxpdb);
  if(ptr== nullptr) {
    return R_NilValue;
  }
  ...
  return db->values_from_calls(pkg_name, fun_name);
}
