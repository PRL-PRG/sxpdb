...
db_from_values <- function(values, origins = NULL, name = "sxpdb", with_search_index = FALSE) {
  ...
  db <- sxpdb::open_db(tempfile(name), mode = TRUE, quiet = TRUE)

  if (is.null(origins)) {
    for (v in values) {
      sxpdb::add_val_origin(db, v, "pkg", "f", "arg")
    }
  } else {
    stopifnot(length(values) == length(origins))
    for (i in seq_along(values)) {
      sxpdb::add_val_origin(
        db, values[[i]], origins[[i]][[1]],
        origins[[i]][[2]], origins[[i]][[3]]
      )
    }
  }
